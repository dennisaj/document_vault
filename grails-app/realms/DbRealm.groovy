    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def retval = permissions?.find { permString ->
            ...
            if (perm.implies(requiredPermission)) {
                // us.paperlesstech.User has the permission!
                return true
            }
            else {
                return false
        ...
        def results = User.executeQuery("select distinct p from us.paperlesstech.User as user join user.roles as role join role.permissions as p where user.username = '$principal'")
