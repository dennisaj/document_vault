...
class NimbleHeaderTagLib {
	...
	def nimblecore = {attrs ->
		out << render(template: "/templates/header/jquery/nimblecore", contextPath: pluginContextPath, model:[nimblePath:pluginContextPath])
	}

	// Enables js for Nimble supplied UI features
	def nimbleui = {attrs ->
		out << render(template: "/templates/header/jquery/nimbleui", contextPath: pluginContextPath, model:[nimblePath:pluginContextPath])
	}

	// Enables growl like message popup in the page
	def growl = {attrs, body ->
		out << render(template: "/templates/header/jquery/growl", contextPath: pluginContextPath, model:[nimblePath:pluginContextPath])
	}

	// Imports JQuery Javascript to make the JQuery library available to the current page
	def jsetup = {attrs, body ->
		out << render(template: "/templates/header/jquery/setup", contextPath: pluginContextPath, model:[nimblePath:pluginContextPath])
	}

	// imports library to do password strength test
	def pstrength = {attrs, body ->
		out << render(template: "/templates/header/jquery/pstrength", contextPath: pluginContextPath, model:[nimblePath:pluginContextPath])
	}
	...
	def login = {attrs, body ->
		out << render(template: "/templates/header/jquery/loginsetup", contextPath: pluginContextPath, model:[nimblePath:pluginContextPath])
	}
