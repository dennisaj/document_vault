...
class AccountController {
	...
	def validateuser = {
		...
		if (!user) {
			...
			flash.type = "error"
			flash.message = g.message(code: 'default.not.found.message', args: [g.message(code: 'nimble.user.label'), params.id])
		...
		if (!user.enabled && user.actionHash != null && user.actionHash.equals(params.activation)) {
			...
			if (user.hasErrors()) {
				...
				flash.type = "error"
				flash.message = g.message(code: 'nimble.user.validate.error')
				redirect controller: 'auth', action: 'login'
				return
			}
			...
			flash.type = "success"
			flash.message = g.message(code: 'nimble.user.validate.success')
			redirect controller: 'auth', action: 'login'
	...
	def forgottenpasswordprocess = {
		...
		if (profile) {
			...
			flash.type = "error"
			flash.message = g.message(code: 'nimble.user.forgottenpassword.noaccount')
