...
(function($) {
    $.widget("ui.tagit", {
        ...
        _create: function() {
            ...
            if ($.isArray(self.options.initialTags)) { 
            ...
            this.options.select = function(event, ui) {
                ...
                self._addTag($.htmlEncode(ui.item.value));
                return false;
            }
            ...
            this.input.keydown(function(e) {
                ...
                if (self._isInitKey(e.which)) {
                    ...
                    if ($(this).val().length >= self.options.minLength)
                        self._addTag($.htmlEncode($(this).val()));
                }
            ...
            if (this.options.select) {
                this.element.after('<select class="tagit-hiddenSelect" name="'+this.element.attr('data-name')+'" multiple="multiple"></select>');
        ...
        _addTag: function(value, skipCallback) {
            this.input.val("");
            value = value.replace(/,+$/, "");
            value = value.trim();
            value = this.options.preserveCase ? value : value.toLowerCase();
        ...
        _initialTags: function() {
            if ($.isFunction(this.options.initialTags)) {
                $.ajax({
                    context: this,
                    url: this.options.initialTags(this.element),
